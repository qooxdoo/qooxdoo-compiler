...
`;
  ...
  await fsPromises.writeFile("tmp/qx", `#!/usr/bin/env node
const path=require("path");
require(path.join(__dirname, "compiled", "node", "build", "compiler"));
`, "utf8");
  await runCommand(".", "chmod", "+x", "tmp/qx");
  ...
  result = await runCommand(".", "./tmp/qx", "compile");
  if (result.exitCode) {
    process.exit(result.exitCode);
  }
  
  console.log("Compiling build version");
  result = await runCommand(".", "./tmp/qx", "compile", "--target=build");
  if (result.exitCode) {
    process.exit(result.exitCode);
  }
