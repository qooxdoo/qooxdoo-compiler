...
qx.Class.define("qx.tool.cli.commands.package.Install", {
  ...
  members: {
    ...
    async __readConfigData() {
      this.__manifest = await qx.tool.utils.Json.loadJsonAsync(qx.tool.config.Manifest.getInstance().getFileName());
    ...
    async __saveConfigData() {
      ...
      if (this.argv.save) {
        let manifest_file = qx.tool.config.Manifest.getInstance().getFileName();
