...
qx.Class.define("qx.tool.compiler.utils.ConfigFile", {
  ...
  members: {
    ...
    _applyData(data, old) {
      ...
      if (dataSchemaInfo.uri !== this.__schemaUri && dataSchemaInfo.version > 0) {
      ...
      try {
        ...
        let msg = `Schema validation error in ${this.__path}: ${e.message}`;
        throw new qx.tool.cli.Utils.UserError(msg);
      }
    },
    ...
    async save() {
      ...
      qx.tool.compiler.utils.Json.validate(data, this.__schemaInfo);
