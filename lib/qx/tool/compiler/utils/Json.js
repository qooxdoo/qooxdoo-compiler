...
qx.Class.define("qx.tool.compiler.utils.Json", {
  ...
  statics: {
     ...
     * has been passed. Return an object containing informatioon on that schema
     * which can be used to validate the data.
     * @param data {Object} Manifest.json or compile.json
     * @return {{type,version,file,url}|null} Object with information about the
     * schema or null if no schema could be detected.
     */
    getSchemaInfo: function(data) {
      ...
      let common = path.join("v" + schemaInfo.version, qx.tool.ConfigSchemas[schemaInfo.type].filename);
      schemaInfo.file = path.join(qx.tool.ConfigSchemas.schemaBaseDir, common);
      schemaInfo.uri = qx.tool.ConfigSchemas.schemaBaseUrl + "/" + common;
      return schemaInfo;
    },
