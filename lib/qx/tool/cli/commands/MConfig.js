  ...
  members: {
    ...
    _mergeArguments: function(parsedArgs, config, contribConfig) {
      ...
      if (parsedArgs.libraries) {
        ...
        await this.__addQxPath(config.libraries,"../../../../../..");
        await this.__addQxPath(config.libraries,"../../../../../../node_modules");
    ...
    __addQxPath: async function(arr,dir){
        let qxDir = path.join(__dirname + dir + '/qooxdoo-sdk/framework');
        if (await fs.existsAsync(qxDir + '/Manifest.json')){
            arr.push(qxDir);
        }
    },
    /**
