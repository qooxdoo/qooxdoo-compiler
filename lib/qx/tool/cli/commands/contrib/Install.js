...
qx.Class.define("qx.tool.cli.commands.contrib.Install", {
  ...
  members: {
    ...
    process: async function() {
      ...
      if (qx.lang.Type.isString(id) && id.startsWith("tree/")) {
        await this.__installFromTreeHash(uri, id, true);
      } else {
     ...
     * @param {String} hash
     *  A tree hash such as "tree/892f44d1d1ae5d65c7dd99b18da6876de2f2a920"
     * @param {Boolean} writeToManifest Whether the library should be written to
     * Manifest.json as a dependency
     * @return {Promise<void>}
     * @private
     */
    __installFromTreeHash: async function(uri, hash, writeToManifest) {
      let qooxdoo_version = await this.getUserQxVersion();
      if (this.argv.verbose) {
     ...
     * @param {String} id
     *  If preceeded by "tree/", the hash of the code tree in github.
     *  Otherwise, the name of a release tag
