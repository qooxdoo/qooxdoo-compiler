  ...
  members: {
    ...
    process: async function() {
      ...
      if (manifest.info["qooxdoo-range"]) {
        if( ! argv.quiet) console.info(`Deprecated key "manifest.info.qooxdoo-range" found, will be changed to manifest.requires.libraries.qx`);
        semver_range = manifest.info["qooxdoo-range"];
        delete manifest.info["qooxdoo-range"];
      }
      semver_range = (manifest.requires && manifest.requires.libraries && manifest.requires.libraries.qx) || semver_range;
      if( ! semver.satisfies( qooxdoo_version, semver_range, {loose: true} ) ){
        ...
        manifest.requires.libraries.qx = semver_range;
      }
