        ...
        for (let repo of result.data) {
          ...
          for (let release of releases_data.data) {
            ...
            for (let [index, { path }] of manifests.entries()) {
              ...
              // @deprecated
              var qx_version_range = (data.requires && data.requires.libraries && data.requires.libraries.qx) || data.info["qooxdoo-range"];
              if (argv.verbose && data.info["qooxdoo-range"] ) {
                  console.warn(`!!! info.qooxdoo-range is deprecated. Please use the requires.libraries.qx key instead.`);
              }  
              ...
              if ( typeof qx_versions == "string" && ! qx_version_range ) {
                ...
                if (argv.verbose) {
                  console.warn(`!!! info.qooxdoo-version is deprecated. Please use the requires.libraries.qx key instead.`);
                }
              } 
              ...
              if ( qx_versions instanceof Array && qx_versions.length && ! qx_version_range ) {
                ...
                if (argv.verbose) {
                  console.warn(`!!! Manifest key 'info.qooxdoo-version' is deprecated. Please use the requires.libraries.qx key instead.`);
                }                
              }             
              ...
              let info = data.info;
              let namespace = data.provides.namespace;
              manifests[index] = { namespace, path, qx_versions, info };
              num_libraries++;
              if (argv.verbose) {
