...
qx.Class.define("qx.tool.cli.commands.package.Install", {
  ...
  members: {
    ...
    async __readConfigData() {
      this.__manifest = await qx.tool.compiler.utils.Json.loadJsonAsync(qx.tool.ConfigSchemas.manifest.filename);
    ...
    __installApplication: async function(downloadPath) {
      let manifest = await qx.tool.compiler.utils.Json.loadJsonAsync(path.join(downloadPath, qx.tool.ConfigSchemas.manifest.filename));
