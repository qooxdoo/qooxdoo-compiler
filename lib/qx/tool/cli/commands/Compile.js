...
qx.Class.define("qx.tool.cli.commands.Compile", {
  ...
  members: {
    ...
    process: async function() {
      ...
      await this.__checkDependencies(maker, config.contribs);
          ...
          for(let req of Object.getOwnPropertyNames(requires)) {
            ...
            switch (req) {
              case "qooxdoo-compiler": {
                ...
                if (!semver.satisfies(qxVer, libVer, {loose: true})) {
                  throw new qx.tool.cli.Utils.UserError(
                    `Library ${lib.getNamespace()} needs qooxdoo-compiler version ${libVer}, found ${qxVer}`
                  );
                }
                break;
              }
              case "qooxdoo-sdk": {
                ...
                if (!semver.satisfies(qxVer, libVer, {loose: true})) {
                  throw new qx.tool.cli.Utils.UserError(
                    `Library ${lib.getNamespace()} needs qooxdoo-sdk version ${libVer}, found ${qxVer}`
                  );
                }
                break;
              }
              default: {
                  let l = libs.find(entry => entry.getLibraryInfo().name == contribs[req]);
                  let qxVer = l.getLibraryInfo().version;
                  if (!semver.satisfies(qxVer, libVer, {loose: true})) {
                    throw new qx.tool.cli.Utils.UserError(
                      `Library ${lib.getNamespace()} needs ${req} version ${libVer}, found ${qxVer}`
                    );
                  }
                }
                break;  
