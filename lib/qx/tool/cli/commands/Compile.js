...
qx.Class.define("qx.tool.cli.commands.Compile", {
  ...
  members: {
    ...
    process: async function() {
      ...
      let errors = await this.__checkDependencies(maker);
      if (errors.length > 0) {
    ...
    createMakerFromConfig: async function(data) {
      ...
      data.libraries.forEach(async libData => {
        let lib = await addLibraryAsync(libData);
      });
    ...
    __checkDependencies: async function(maker) {
      let errors = [];
      ...
      libs.forEach(lib =>  {
        ...
        if (range) {
          console.warn(`${lib.getNamespace()}: The configuration setting qooxdoo-range in manifest.json is deprecated (move to requires.libraries.qx)`);
          if (!requires ) {
             requires = {};  
          }
          if (!requires.libraries ) {
            requires.libraries = {};  
         }
         if (!requires.libraries.qx) {
            requires.libraries.qx = range;
          }
        }
        if (requires) {
          let qxVer = qx.tool.compiler.Version.VERSION;
          let libVer = requires["qooxdoo-compiler"];
          if (libVer && !semver.satisfies(qxVer, libVer, {loose: true})) {
            errors.push(`${lib.getNamespace()}: Needs qooxdoo-compiler version ${libVer}, found ${qxVer}`);
          }
          if (requires.libraries) {
            for(let req of Object.getOwnPropertyNames(requires.libraries)) {
              let libVer = requires.libraries[req];
              let l = libs.find(entry => entry.getNamespace() === req);
              if (!l) {
                errors.push(`${lib.getNamespace()}: ${req} not found. Check Manifest.json and contrib.json`);
              } else {
    ...
    resolveTargetClass: function(type) {
      ...
      if (type == "typescript")
         throw qx.tool.cli.Utils.UserError("Typescript targets are no longer supported - please use `typescript: true` in source target instead");
      if (type) {
        var targetClass;
        if (type.indexOf('.') < 0)
